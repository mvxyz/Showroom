...
//
Процедура ВыполнитьДействияПриЗапускеСистемы(ЭтоЧистаяБаза) Экспорт
	...
	Если ЭтоЧистаяБаза 
		...
		ОбновлениеИнформационнойБазыКлиент.ВыполнитьОбновлениеИнформационнойБазы(ЕстьОшибкиПриОбновлении);
...
// Выполняет открытие раздела "Номенклатура"
//
// Параметры:
//  ПараметрыВыполненияКоманды	 - ПараметрыВыполненияКоманды - Структура параметров общей команды.
//
Процедура ОткрытьРазделНоменклатура() Экспорт
	
	ОткрытьФорму("Справочник.Номенклатура.ФормаСписка",,,,,,,РежимОткрытияОкнаФормы.Независимый);	

КонецПроцедуры

// Выполняет открытие раздела "Контрагенты"
//
Процедура ОткрытьРазделКонтрагенты() Экспорт
	
	ОткрытьФорму("Справочник.Контрагенты.ФормаСписка",,,,,,,РежимОткрытияОкнаФормы.Независимый);	

КонецПроцедуры
	
// Выполняет открытие раздела "Заказы"
//
Процедура ОткрытьРазделЗаказы() Экспорт
	
	ОткрытьФорму("Документ.Заказ.ФормаСписка",,,,,,,РежимОткрытияОкнаФормы.Независимый);	

КонецПроцедуры
	
// Выполняет открытие раздела "Корзина"
//
Процедура ОткрытьРазделКорзина() Экспорт
	...
	ПараметрыФормы = Новый Структура("Ключ", Корзина);

	ОткрытьФорму("Справочник.Корзины.ФормаОбъекта", ПараметрыФормы,,,,,,РежимОткрытияОкнаФормы.Независимый);	

КонецПроцедуры

// Возвращает форму Корзина текущего пользователя
//
Функция ПолучитьФормуКорзина() Экспорт
	...
	ФормаКорзина = ПолучитьФорму("Справочник.Корзины.ФормаОбъекта", ПараметрыФормы);
	
	Возврат ФормаКорзина;
...
Процедура ОтсканироватьШтрихкод(ОбработчикСканирования, Форма) Экспорт
	...
	#Если МобильноеПриложениеКлиент ИЛИ МобильныйКлиент Тогда
		...
		Иначе
			Предупреждение(НСтр("ru='На этом устройстве сканирование штрихкодов не поддерживаются.';en='In this barcode scanning device is not supported.'"),,ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьСинонимКонфигурации());
		КонецЕсли;
	#Иначе
		//Штрихкод = "";
...
&НаКлиенте
Функция ТекущийТоварУжеЕстьВЗаказе(Номенклатура, ХарактеристикаНоменклатуры, ТоварыЗаказа, ПоказыватьСообщение = Истина) Экспорт
	
	ТекущийТоварУжеЕстьВЗаказе = Ложь;
	
	Для каждого ТоварЗаказа Из ТоварыЗаказа Цикл
		Если ТоварЗаказа.Номенклатура = Номенклатура
			И ТоварЗаказа.ХарактеристикаНоменклатуры = ХарактеристикаНоменклатуры Тогда
			ТекущийТоварУжеЕстьВЗаказе = Истина;
		КонецЕсли;
	КонецЦикла;
	
	Если ТекущийТоварУжеЕстьВЗаказе И ПоказыватьСообщение Тогда
		ТекстСообщения = "Товар '" + СокрЛП(Номенклатура)
		+ ?(ЗначениеЗаполнено(ХарактеристикаНоменклатуры), " ("+СокрЛП(ХарактеристикаНоменклатуры)+")","") 
		+ "' уже есть в заказе!";
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	
	Возврат ТекущийТоварУжеЕстьВЗаказе;
